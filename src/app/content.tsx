...
const Content = () => {
  ...
  const handleSearchSubmit = async () => {
    setLoading(true);
    const resp = await axios.get(url.repos(search));

    const updatedUsers = [...users];
    const newUser: { username: string, repos: Array<object> } = {
      username: search,
      repos: resp.data
    };
  ...
  return (
    <div className='border border-zinc-300 p-8 rounded'>
      ...
      <div>
        <button type='button' className='text-white bg-blue-500 hover:bg-blue-600 rounded p-2 w-100 cursor-pointer text-center' onClick={handleSearchSubmit}>{loading ? loadingAnimation() : 'Search'}</button>
      </div>
      {users.length > 0 && (
        <p className='text-zinc-500 mt-5'>Showing users for</p>
      )}
    </div>
  )
}
